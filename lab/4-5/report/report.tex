...
\begin{markdown}
Looking into the buffer resizing policy of the OS might help to explain the behaviours recorded. From figure \ref{fig:10ms_wnd} it is clear that the resizing happens adding a constant step to the current buffer size. This is confirmed by FreeBSD 11 code. From \cite{tcp\_input} it is possible to see that the variable `tcp_autorcvbuf_inc` is defined as:
~~~c
...
With this new buffer resizing policy, it is likely the results were different, with a smaller gap between the two policies at higher latencies.
\end{markdown}
